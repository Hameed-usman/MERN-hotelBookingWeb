...
const RadioButton = ({ label, selected = false, onChange = () => {} }) => {
  return (
    <label className='flex gap-3 items-center cursor-pointer mt-2 text-sm'>
      <input type="radio" name='sort option' checked={selected} onChange={() => onChange(label)} />
      <span className='font-light select-none'>{label}</span>
    </label>
  )
}
...
const AllRooms = () => {
  ...
  return (
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:-px-32'>
      <div className=''>
        ...
        {roomsDummyData.map((room) => (
          <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'> 
            <img onClick={() => { Navigate(`/rooms/${room._id}`); scrollTo(0, 0) }} src={room.images[0]} alt="Hotel-Img" title='View Room Details' className='max-h-65 md:w-1/2
              rounded-2xl shadow-lg object-cover cursor-pointer' />
              ...
              <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                {room.amenities.map((item, index) => (
                  <div key={index} className='flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F5F5FF]/70'>
                    <img className='w-4 h-5' src={facilityIcons[item]} alt={item} />
                    <p className='px-3 text-xs'>{item}</p>
                  </div>
                ))}
              </div>
      ...
      {/* FILTERS */}
      <div className='bg-white w-80 border border-gray-700 text-gray-600 max-lg:mb-8 min-lg:mt16'>
